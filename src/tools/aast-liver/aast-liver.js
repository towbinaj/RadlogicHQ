...
function init() {
  trackEvent('tool:aast-liver:opens');
  ...
  function buildUI() {
    ...
    // Render each category as a card with toggle buttons
    for (const cat of definition.categories) {
      ...
      card.querySelectorAll('.benign-choice').forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          if (formState.selectedFindings.has(fid)) {
            formState.selectedFindings.delete(fid);
            btn.classList.remove('benign-choice--active');
          } else {
            formState.selectedFindings.add(fid);
    ...
    multiCard.innerHTML = `
      <label class="aast-multi-check">
        <input type="checkbox" id="multi-injuries" ${formState.multipleInjuries ? 'checked' : ''}>
        Multiple Grade I/II injuries present (advances to Grade III)
      </label>
    `;
    multiCard.querySelector('#multi-injuries').addEventListener('change', (e) => {
      formState.multipleInjuries = e.target.checked;
      update();
    });
  ...
  parseBtn.addEventListener('click', () => {
    ...
    if (!text) return;
    const { formState: parsed, matched, unmatched, remainder } = parseFindings(text, aastLiverDefinition);
