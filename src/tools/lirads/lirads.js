...
function init() {
  ...
  parseBtn.addEventListener('click', () => {
    ...
    // Item-indexed parsing: "Observation 1: ... Observation 2: ..." (or
    // numbered "1. ... 2. ...") pastes split into per-observation segments,
    // each becoming its own tab. Single-observation pastes with no marker
    // fall through to the ungrouped bucket and apply to the active tab.
    const { segments, ungrouped, unmatchedSentences } = parseSegmentedFindings(text, liradsDefinition);

    let matchedFieldCount = 0;

    if (segments.length > 0) {
      // Multi-observation paste: rebuild observations from segments. Use
      // each segment's detected index for the label so "Observation 2: ..."
      // keeps its "Observation 2" label even if it was first in the paste.
      observations = segments.map((seg) => ({
        id: seg.index,
        label: `Observation ${seg.index}`,
      ...
      // Single-observation paste: replace active obs's form state (old
      // single-obs behavior).
      const fs = observations[activeObsIndex].formState;
    ...
    // Additional Findings. Drop bare index markers ("1.", "(2)") that leak
    // through the sentence splitter when numbered-line markers were used.
    ...
    const obsCount = segments.length > 0 ? segments.length : 1;
    const obsSuffix = obsCount > 1 ? ` across ${obsCount} observations` : '';
    parseStatus.textContent = `Matched ${matchedFieldCount} field(s)${obsSuffix}${unmatchedSentences.length ? ' \u2014 remainder in Additional Findings' : ''}`;
